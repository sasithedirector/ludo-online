...
// Board constants (must match server)
...
export const HOME_CELLS: Record<Color, [number, number][]> = {
  red:    [[1,1],[1,4],[4,1],[4,4]],
...
export function getTokenPosition(color: string, pos: number): [number, number] | null {
  if (pos === -1) return null; // in home, handled separately
  if (pos < 52) return PATH[pos];
  if (pos < 58) return HOME_STRETCH[color as Color]?.[pos - 52] || null;
